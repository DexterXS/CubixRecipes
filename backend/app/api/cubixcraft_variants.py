...
def _load(path: Path) -> list[dict[str, Any]]:
    ...
    for item in payload:
        if not isinstance(item, dict):
            continue
        clone = dict(item)
        clone['active'] = bool(clone.get('active', False))
        result.append(clone)
    return result
...
def _normalize_source(source: str) -> str:
    return source.replace('// CubixRecipes:preferred\n', '').replace('// CubixRecipes:preferred\r\n', '').strip()
...
def _upsert(variants: list[dict[str, Any]], file_path: str, output: str, source: str, active: bool) -> dict[str, Any]:
...
@router.post('')
async def save_variant(request: Request):
    _require_editor(request)
    ...
    variants = _load(path)
    variant = _upsert(variants, file_path, output, source, active)
    _save(path, variants)
    return {'ok': True, 'variant': variant}


@router.post('/sync')
async def sync_published_variants(request: Request):
    _require_editor(request)
    payload = await request.json()
    file_path = str(payload.get('filePath') or '').strip()
    recipes = payload.get('recipes')
    if not file_path or not isinstance(recipes, list):
        raise HTTPException(status_code=400, detail='filePath and recipes are required')
    ...
    for recipe in recipes:
        if not isinstance(recipe, dict):
            continue
        output = str(recipe.get('output') or '').strip()
        source = str(recipe.get('source') or '').strip()
        if not output or not source:
            continue
        synced.append(_upsert(variants, file_path, output, source, True))
    _save(path, variants)
    return {'ok': True, 'variants': synced}


@router.patch('/{variant_id}')
async def set_variant_state(variant_id: str, request: Request):
    _require_editor(request)
    payload = await request.json()
    ...
    path = _store_path(request)
    variants = _load(path)
    target = next((item for item in variants if item.get('id') == variant_id), None)
    if target is None:
        raise HTTPException(status_code=404, detail='CubixCraft variant not found')
