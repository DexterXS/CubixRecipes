...
- Для локального управления можно запустить `python start-dev.py` из корня проекта: откроется Tkinter-панель с кнопками Start/Stop/Restart для backend и frontend. В нижней части окна есть журнал действий, который объясняет, что именно запускается или останавливается, почему это происходит и какие процессы сейчас доступны для остановки/перезапуска. Для frontend на Windows запуск идёт через `cmd /k npm run dev`, поэтому если Vite или зависимости падают, консоль остаётся открытой и показывает причину ошибки. На Windows процессы открываются в отдельных консолях; скрипт ожидает, что зависимости уже установлены.
