...
- Для локального управления можно запустить `python start-dev.py` из корня проекта: откроется Tkinter-панель с кнопками Start/Stop/Restart для backend и frontend. На Windows процессы открываются в отдельных консолях; скрипт ожидает, что зависимости уже установлены.
