...
- Optimized Auctions graph dragging by rendering movement through a local SVG preview, preserving the pointer grab offset, committing the final pointer-up position, keeping percentage changes when an invalid day drop snaps back, and running the placement check only once on pointer release.
