...
- Добавил ПКМ по вкладке избранного для смены имени и иконки из доступных предметов NEI; имя вкладки также остаётся доступно при наведении.
- Нормализовал размер иконок вкладок и убрал лишние успешные сообщения «Настройки сохранены» и «Избранное сохранено».
- Исправил компоновку объединённого окна: избранное и черновики слева, NEI и крафты выбранного черновика справа теперь идут независимыми вертикальными стеками; нижняя левая панель больше не ждёт высоту правого NEI, а центр получил больше ширины.
- Уплотнил объединённое окно крафтов: боковые панели теперь занимают высоту по содержимому и идут сверху последовательно, их колонки подстраиваются под набор предметов, а центральный редактор и список файлов больше не растягиваются пустым пространством; глобальная верхняя навигация стала тоньше.
...
- Added a server-and-user scoped browser bootstrap cache for NEI favorites: the last confirmed profile is restored immediately after F5, persisted in localStorage with an IndexedDB copy, and refreshed from the backend in the background.
- Added a server-and-user scoped IndexedDB snapshot for the full NEI item catalog, so the catalog can be restored before the backend refresh even when the large localStorage snapshot cannot fit in quota.
...
- Completed lazy mod-atlas page caching: the browser stores only the manifest at startup and Cache Storage captures x32/x256 (and future) atlas pages on their first real request, avoiding eager downloads of unused pages.
